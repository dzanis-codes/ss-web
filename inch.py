...
def glabat_slud(link, type, type_count):
    print(link)
...
#vajag range pareizi saskaitīt
    for count in range(len(g_data)):
        ...
        type = type_list[type_count]


        ts = time.gmtime()
        stamp = time.strftime("%Y-%m-%d %H:%M:%S", ts)

        sql_entry = (str(ad_link), str(stavi_kopa), str(adrese), str(platiba), "land?", str(cena), str(cena_m2), "inch.lv", str(transaction), str(type), str(istabas), stamp) 
        print(sql_entry)
...
while link != len(link_list):
    try:
        ...
        type_count = 0
        while type_count != len(type_list):
          place = link_list[link].find('apartment')
          part1 = link_list[link][:place]
          part2 = link_list[link][place + 9:]
          newlink = part1 + type_list[type_count] + part2
          print(newlink)
          glabat_slud(newlink, link, type_count)
          time.sleep(5)
